...
def finite_difference(temp_grid, 
                     heat_deposition, 
                     pulse_amplitude,  
                     delta_x,
                     delta_y,
    ...
    heat_constant = delta_t / (rho_0 * C_t)
    ...
    returning_temp[1:Nz-1, 1:Ny-1, 1:Nx-1] = (heat_deposition[1:Nz-1, 1:Ny-1, 1:Nx-1] * (pulse_amplitude ** 2) * heat_constant + 
...
def rectangular_pulse(time, period_duration = 2e-3, period_repetition_interval = 4e-3):
...
def turk_pulse(time, period_duration = 3.25e-3, period_repetition_interval = 4e-3, ramp_duration = 1e-3):
